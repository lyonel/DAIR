...
CREATE VIEW tags AS SELECT keyword AS tag,entryid FROM keywords UNION SELECT category AS tag,id AS entryid FROM entries UNION SELECT project AS tag,id AS entryid FROM entries UNION SELECT ' flagged',id AS entryid FROM entries WHERE DATE('now')>=flagdate UNION SELECT * FROM deadlines;
