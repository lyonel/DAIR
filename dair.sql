...
CREATE VIEW deadlines AS SELECT ' overdue' AS tag,ROWID AS entryid FROM entries WHERE open AND (DATE(deadline)<=DATE('now','localtime')) UNION SELECT ' due this month' AS tag,ROWID AS entryid FROM entries WHERE open AND (DATE(deadline)>=DATE('now', 'localtime', 'start of month')) AND (DATE(deadline)<DATE('now', 'localtime', 'start of month', '+1 month')) UNION SELECT ' due  this week' AS tag,ROWID AS entryid FROM entries WHERE open AND (DATE(deadline)>=DATE('now', 'localtime', 'weekday 0', '-7 day')) AND (DATE(deadline)<DATE('now', 'localtime', 'weekday 0')) UNION SELECT ' due next week' AS tag,ROWID AS entryid FROM entries WHERE open AND (DATE(deadline)>=DATE('now', 'localtime', 'weekday 0')) AND (DATE(deadline)<DATE('now', 'localtime', 'weekday 0', '+7 day'));
CREATE VIEW persons AS SELECT owner AS person,ROWID AS entryid FROM entries UNION SELECT author AS person,entryid FROM notes UNION SELECT ' no owner'AS tag,ROWID AS entryid FROM entries WHERE open AND owner IS NULL;
CREATE VIEW tags AS SELECT keyword AS tag,entryid FROM keywords UNION SELECT category AS tag,ROWID AS entryid FROM entries UNION SELECT project AS tag,ROWID AS entryid FROM entries UNION SELECT ' flagged',ROWID AS entryid FROM entries WHERE flagged UNION SELECT * FROM deadlines;
